...
function build_comment( $arr, $comment_reply = false)
{
	global $app_module;
	$html = "\n" . '<ul'.$comment_reply.'>'."\r\n";
	...
	foreach ($arr as $key => $val) 
	{
		...
		$has_child = key_exists('children', $val) ? ' class="has-reply" ' : '';
		
		// Menu
		$html .= '<li'. $has_child . ' id="li-comment-'.$val['comment_id'].'">
					<article class="comment-body">
						<section class="comment-content">
							<div class="comment-meta">
								<a href="#" class="author">' . $val['name'] . '</a>
								<a href="#" class="date">' . $val['comment_date'] . '</a>
							</div>
								<p>
							' . $val['comment'] . '
							</p>
						</section>
						<section class="footer"><a class="reply-comment" id="reply-'.$val['comment_id'].'" href="javascript:void(0)"><i class="fas fa-reply"></i>&nbsp;Reply</a></section>
					</article>
					';
		
		if (key_exists('children', $val))
		{ 	
			$html .= build_comment($val['children'], ' class="comment-reply"');
		} 
		$html .= "</li>\n";
	}
	$html .= "</ul>\n";
